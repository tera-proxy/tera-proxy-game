...
class Dispatch extends EventEmitter {
	...
	hook(moduleName, name, version, opts, cb) {
		// parse args
		...
		if(name === '*') { // wildcard
			...
			if(code == null) {
				log.error(`[dispatch] hook: unrecognized hook target ${getMessageName(this.protocolMap, name, version, name)}\n${errStack()}`)
				return null
			}

			// check version - TODO: replace with proper deprecation
			if(version !== '*') {
				const latest = latestDefVersion.get(name)
				if(latest && version < latest)
					log.warn(`[dispatch] hook: ${getMessageName(this.protocolMap, name, version, name)} is not latest version (${latest})\n${errStack()}`)
			}
		}
		...
		const order = opts.order || 0,
			hook = {
				moduleName, // TODO: replace with wrapper ref
				code,
